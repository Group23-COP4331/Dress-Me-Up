...
export default function RegisterForm({setMessage} : RegisterFormComponent){
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(buildPath('api/register'), { //call buildPath to make the api route dynamic it will make a string depending on if we are on local host or server
        method: 'POST',
  ...
  title="Password must be at least 8 characters long, contain one letter, one number, and one symbol." id="password" name="password" placeholder='Password'/>
      </div>
